...
```markdown
ロブスター[^1] は甲殻類でもあり、Markdown パーサー[^parser] でもあります。

[^1]: 甲殻類は 10 本の脚を持ち、海に住んでいます。
[^parser]: パーサーは脚が 0 本で、あなたのブラウザに住んでいます。
```
...
```markdown
CDN の URL^[https://hacknock.github.io/lobsterjs/lobster.js] は常に最新です。
...
[^api]: 完全な API リファレンスについては、はじめにのページをご覧ください。
```
